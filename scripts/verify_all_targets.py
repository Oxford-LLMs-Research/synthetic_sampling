#!/usr/bin/env python3
"""Final verification that all 261 targets are predicted in results."""
...
extra = result_targets - dataset_targets
match = dataset_targets & result_targets

print("=" * 70)
print("FINAL VERIFICATION REPORT")
print("=" * 70)
print(f"\nDataset description: {len(dataset_targets)} targets")
print(f"Model results (fixed parsing): {len(result_targets)} targets")
print(f"\nMatching targets: {len(match)}")
print(f"Missing from results: {len(missing)}")
print(f"Extra in results: {len(extra)}")

print(f"\n" + "=" * 70)
if len(missing) == 0 and len(extra) == 0:
    print("[VERIFIED] All 261 targets have predictions in results!")
    print("=" * 70)
    print(f"\nTotal instances: {len(all_instances):,}")
    print(f"All {len(result_targets)} target questions are present with predictions.")
else:
